...
// Erkennt automatisch, ob "image" ein echter Bildpfad ist (z.B. "images/cpu.png")
// oder noch ein Platzhaltertext wie "[BILD_CPU]". Bei einem Pfad wird ein
// <img>-Tag eingefügt, sonst der Platzhaltertext wie gewohnt angezeigt.
function renderQuestionImage(containerEl, imageValue) {
  const looksLikePath = /\.(png|jpe?g|gif|webp|svg)$/i.test(imageValue.trim());
  if (looksLikePath) {
    containerEl.innerHTML = `<img src="${escapeHtml(imageValue.trim())}" alt="Frage-Bild" style="max-width:100%; max-height:220px; border-radius:10px;" onerror="this.parentElement.textContent='⚠️ Bild nicht gefunden: ${escapeHtml(imageValue)}'" />`;
  } else {
    containerEl.textContent = imageValue;
  }
}

// ---------- Verfügbare Avatare/Emojis für die Spieler-Auswahl beim Beitritt ----------
